...
import { constructionDashboardService } from '@/services/api'
...
export default function ConstructionPresentationDashboard() {
  ...
  // Fetch construction data on mount
  useEffect(() => {
    const fetchConstructionData = async () => {
      try {
        ...
        const summaryRes = await constructionDashboardService.getConstructionProjects()
        setProjectSummary(summaryRes.data)

        // Fetch project list
        const listRes = await constructionDashboardService.getConstructionProjects()
        setProjectList(listRes.data?.projects)

        // Fetch BOQ summary
        const boqRes = await constructionDashboardService.getBoqSummary()
        setBOQSummary(boqRes.data)

        // Fetch timeline
        const timelineRes = await constructionDashboardService.getProjectTimeline()
  ...
  const activeProjects = projectSummary?.active_projects || 12
  const avgCompletion = projectSummary?.avg_completion || 68
  const totalBOQ = boqSummary?.total_boq || 4580000000
  const executedValue = boqSummary?.executed_value || 3120000000
  const remainingBudget = totalBOQ - executedValue
      ...
      content: (
        <div className="flex flex-col items-center justify-center h-full gap-8">
          <div className="text-center">
            ...
            <p className="text-2xl text-gray-700 mb-2">December 2024</p>
            <p className="text-xl text-gray-600">Multi-Project Overview</p>
          </div>
          <div className="grid grid-cols-2 gap-6 mt-8 w-full max-w-2xl">
            <StatBox title="Active Projects" value={activeProjects.toString()} icon="🏢" />
            <StatBox title="Avg Completion" value={`${avgCompletion}%`} icon="📊" />
          </div>
      ...
      content: (
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-4">
            <StatBox title="Total BOQ Value" value={formatCurrency(totalBOQ)} icon="💰" subtitle="All projects combined" />
            <StatBox title="Executed Value" value={formatCurrency(executedValue)} icon="✓" subtitle={`${Math.round((executedValue / totalBOQ) * 100)}% of total BOQ`} />
            <StatBox title="Remaining Budget" value={formatCurrency(remainingBudget)} icon="📈" subtitle={`${Math.round((remainingBudget / totalBOQ) * 100)}% balance available`} />
          </div>
            ...
            <div className="space-y-3">
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-bold text-gray-900">{formatCurrency(1850000000)} / {formatCurrency(2000000000)}</span>
                </div>
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-bold text-gray-900">{formatCurrency(920000000)} / {formatCurrency(1500000000)}</span>
                </div>
                <div className="w-full bg-gray-300 h-2 rounded-full overflow-hidden">
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-bold text-gray-900">{formatCurrency(350000000)} / {formatCurrency(500000000)}</span>
                </div>
                <div className="w-full bg-gray-300 h-2 rounded-full overflow-hidden">
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-bold text-gray-900">{formatCurrency(240000000)} / {formatCurrency(580000000)}</span>
                </div>
                <div className="w-full bg-gray-300 h-2 rounded-full overflow-hidden">
    ...
    },
    {
      ...
      content: (
        <div className="space-y-6">
          {[
            { title: 'Site Mobilization & Leveling', months: 2, completed: 2 },
            { title: 'Foundation & Pile Works', months: 4, completed: 3.5 },
            { title: 'Structural Steel/RCC Work', months: 6, completed: 3 },
            { title: 'MEP Installation', months: 5, completed: 1 },
            { title: 'Interior Finishing', months: 4, completed: 0 },
            { title: 'Testing & Commissioning', months: 2, completed: 0 }
          ].map((milestone, idx) => {
            const colors = ['bg-green-500', 'bg-green-500', 'bg-yellow-500', 'bg-orange-500', 'bg-gray-300', 'bg-gray-300']
            return (
              <div key={idx}>
                  ...
                  {Array.from({ length: milestone.months }).map((_, i) => (
                    <div
                      ...
                      className={`flex-1 h-4 rounded ${
                        i < milestone.completed ? colors[idx] : 'bg-gray-200'
                      }`}
                    />
                  ))}
                </div>
      ...
      content: (
        <div className="space-y-4">
          <div className="bg-green-50 border-l-4 border-green-500 p-6 rounded-lg">
            <p className="font-bold text-green-900 mb-2">✓ Overall Progress: {avgCompletion}%</p>
            <p className="text-gray-700 text-sm">Projects are progressing well with overall {avgCompletion}% completion. Most projects tracking on schedule.</p>
          </div>

          <div className="bg-green-50 border-l-4 border-green-500 p-6 rounded-lg">
            <p className="font-bold text-green-900 mb-2">✓ Budget Status: On Track</p>
            <p className="text-gray-700 text-sm">Executed {formatCurrency(executedValue)} out of {formatCurrency(totalBOQ)} BOQ. Cost overrun controlled at -2.1%.</p>
          </div>
