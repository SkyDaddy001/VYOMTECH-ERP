...
func (s *SalesAnalyticsService) GetMonthlySalesAnalysis(ctx context.Context, tenantID string, startDate, endDate *time.Time) ([]*models.SalesMonthlySummary, error) {
	query := `
		...
		SUM(u.uds_sqft) as total_uds,
		SUM(CASE WHEN u.status = 'available' THEN u.uds_sqft ELSE 0 END) as unsold_uds,
		SUM(CASE WHEN b.status = 'active' THEN u.uds_sqft ELSE 0 END) as sold_uds,
